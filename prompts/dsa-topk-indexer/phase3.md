# DSA TopK Indexer Phase 3 Prompt
...
Analyze the full workload distribution and specialize the implementation for different shape groups. The main variable axes are `batch_size` and `max_num_pages`; the dataset includes tiny cases such as `batch_size=1, max_num_pages=1`, mid-sized cases around `batch_size=8, max_num_pages=32`, and long-context/high-batch cases up to roughly `batch_size=30, max_num_pages=91`.

Design dispatch logic and specialized kernels only where the measured win justifies the added complexity. Preserve correctness for all 128 workloads. The final candidate should be evaluated on the full workload set, not only on the eight development workloads.
