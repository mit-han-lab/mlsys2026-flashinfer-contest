...
Analyze the full workload distribution and specialize the CUDA implementation for different `seq_len` groups. The workload set includes tiny decode-like cases, mid-sized cases around 52-80 tokens, a 901-token prefill case, and very large prefill cases around 12k-14k tokens.

Design dispatch logic and specialized kernels only where the measured win justifies the added complexity. Consider separate paths for tiny `seq_len`, mid-sized prefill, and large prefill if profiling shows different bottlenecks. The final candidate should be evaluated on the full 19-workload set, not only on the eight development workloads.
